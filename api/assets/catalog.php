...
try {
    ...
    $q = "SELECT a.Asset_ID, a.asset_name, a.description, a.asset_type, a.meetup_location, a.proposed_penalty_amount, a.daily_penalty, a.penalty_type, a.availability, a.status, a.time_created, a.asset_image,
                 ...
                 CASE
                   WHEN a.available_from IS NOT NULL AND a.available_from > NOW() THEN 'upcoming'
                   ELSE 'available'
          ...
          WHERE LOWER(a.status) = 'approved'
            AND (
              -- currently available
              (LOWER(a.availability) = 'available' AND (a.available_until IS NULL OR a.available_until > NOW()))
              OR
              -- upcoming scheduled availability (still requestable)
              (a.available_from IS NOT NULL AND a.available_from > NOW() AND (a.available_until IS NULL OR a.available_until > a.available_from))
            )
    ...
    while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
        $items[] = [
            ...
            "catalog_state" => $row['catalog_state'] ?? 'available',
