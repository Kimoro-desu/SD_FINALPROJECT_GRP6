...
if (!empty($data->transaction_id)) {
    ...
    try {
        ...
        $new_status = Database::STATUS_RETURN_PENDING;
        ...
        // Set to Return_Pending — admin must review before finalizing
        ...
        // DO NOT release the asset yet — admin approval will do that

        // ── Notifications ──
        if ((int)$borrower_id > 0) {
            pushUserNotification($db, (int)$borrower_id, 'Return Submitted for Review', 'Your return of "' . $asset_name . '" has been submitted and is pending admin review.', 'info', 'ph-clock');
        }
        
        if ((int)$lender_id > 0) {
            pushUserNotification($db, (int)$lender_id, 'Return Request Pending', 'A borrower has submitted a return for your asset "' . $asset_name . '". Awaiting admin approval.', 'info', 'ph-clock');
        }
