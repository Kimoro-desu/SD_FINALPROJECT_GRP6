...
<body data-role="borrower">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            setInterval(checkDueDates, 300000); // Check due dates every 5 minutes
        });

        async function checkDueDates() {
            try {
                await window.api.authenticatedFetch('/transactions/check_due_dates.php', { method: 'POST' });
            } catch (e) {
                console.log('Due date check skipped:', e?.message || '');
            }
        }
        ...
        async function fetchDashboardStats() {
            try {
                ...
                if (response && response.status === 'success') {
                    ...
                    if (lenderStatsEl) {
                        lenderStatsEl.textContent = `You have ${stats.pending_requests} pending request${stats.pending_requests !== 1 ? 's' : ''} to review.`;
