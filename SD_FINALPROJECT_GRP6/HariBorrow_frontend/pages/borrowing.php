...
<body>
  ...
  <script>
    ...
    function getStatusData(tx) {
      ...
      if (raw === 'returned' || (raw !== 'return_pending' && tx?.dates?.returned)) return { label: 'Returned', class: 'active' };
      if (raw === 'return_pending') return { label: 'Return Pending', class: 'pending' };
        ...
        if (response && response.status === 'success') {
          ...
          assets = response.assets.map(a => {
            return {
              ...
              asset_image: a.asset_image || null,
    ...
    function renderCatalog() {
      ...
      grid.innerHTML = filteredAssets.map(a => {
        ...
        const locationHtml = a.meetup_location
          ...
          : '';
        const assetVisual = a.asset_image
          ? `<img class="card-image" src="${a.asset_image}" alt="${a.name}" onerror="this.outerHTML='<div class=card-icon><i class=ph \x27' + '${a.icon}' + '\x27></i></div>'">`
