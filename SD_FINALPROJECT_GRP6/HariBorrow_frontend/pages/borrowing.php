...
<body>
  ...
  <main class="page">
    <div class="page-header">
      <div class="page-header-left">
        <h1>Asset Catalog</h1>
        <p>Browse available university equipment and submit borrow requests.</p>
      </div>
      <button class="my-borrows-btn" onclick="openBorrows()">
  ...
  <script>
    ...
    function normalizeBorrowStatus(tx) {
      const raw = String(tx?.status || '').toLowerCase();
      if (tx?.dates?.returned) return 'Returned';
      if (tx?.is_overdue) return 'Overdue';
    ...
    async function loadMyBorrowings() {
      try {
        ...
        // Show active + pending as quick "My Borrowings" indicator.
        const openCount = mine.filter(tx => {
          const st = String(tx?.status || '').toLowerCase();
          const returned = Boolean(tx?.dates?.returned);
          return ((st === 'approved' || st === 'confirmed') && !returned) || st === 'pending';
        }).length;
  ...
  `).join('');
    ...
    async function submitBorrowRequest() {
      ...
      if (!borrowRaw || !returnRaw) {
        alert('Borrow date and return date are required.');
        return;
      }
    ...
    function openBorrows() {
      if (!myBorrowings.length) {
        alert('You have no borrow requests yet.');
        return;
      }
      const lines = myBorrowings.slice(0, 10).map(tx => {
        const status = normalizeBorrowStatus(tx);
        ...
        const parts = [
          `• ${item} — ${status}`,
          `  Borrow Date: ${fmtDateSafe(borrowedAt)}`,
          `  Return Due: ${fmtDateSafe(dueAt)}`,
          `  Returned At: ${fmtDateSafe(returnedAt)}`,
          `  Penalty: PHP ${penalty.toFixed(2)}`
        ];
        return parts.join('\n');
      });
      alert(`My Borrowings (latest)\n\n${lines.join('\n')}`);
    }
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        await loadCatalog();
        await loadMyBorrowings();
        // Keep My Borrowings count/status fresh after approvals.
        setInterval(loadMyBorrowings, 15000);
    });
